...
namespace WebApplication2.ViewModels
{
    public class FacultyProfileViewModel
    {
        ...
        //[Display(Name = "Preferred Departure Airport")]
        //public string PreferredAirport { get; set; }

        //[Display(Name = "Preferred Train Station")]
        //public string PreferredTrainStation { get; set; }

        //[Display(Name = "Preferred Rental Car Company")]
        ...
        public virtual bool TravelProfileComplete()
        {
            if (BirthDate == null ||
                FirstName == null ||
                LastName == null ||
        ...
        public virtual bool DisclosureUpToDate()
        {
            if (Disclosures == null)
            {
                return false;
            ...
            if (Disclosures.ToList().Count < 1)
            {
                return false;
            }
            else
            {
                var x = Disclosures.Where(m => m.IsBackup == false).Single();
                if (x.CreateDate.AddMonths(12) <= DateTime.Now)
                {
                    return false;
                }
